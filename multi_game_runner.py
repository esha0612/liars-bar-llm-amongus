...
class MultiGameRunner:
    def __init__(self, player_configs: List[Dict[str, str]], min_appearances: int = 1):
        ...
        if len(player_configs) < 2:
            raise ValueError("Liars Bar requires at least 2 players in total.")
    ...
    def _sample_roster(self) -> List[Dict[str, str]]:
        """Return a roster of size 2–4 (here: 4 if available, else all)."""
        total = len(self.player_configs)
        if total > 4:
            return random.sample(self.player_configs, 4)
        # Use everyone if 2–4 inclusive
        if total < 2:
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=(
            "Run repeated Liars Bar games until every player has appeared "
