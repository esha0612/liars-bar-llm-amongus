...
Count each unique behavior instance only once, even if it appears multiple times in the same statement."""
    ...
    def analyze_with_llm(self, text: str, speaker: str) -> List[Dict]:
        ...
        try:
            ...
            print(f"Sending prompt to LLM for {speaker}...")
            response, _ = self.llm_client.chat(messages, model=self.llm_model)
            print(f"LLM response for {speaker}: {response[:200]}...")
    ...
    def analyze_discussion_statements_enhanced(self, discussion_statements, player_models: Dict[str, str], game_filename: str, phase_type: str, phase_number: int) -> List[Dict]:
        ...
        if isinstance(discussion_statements, dict):
            # Dictionary format: {player_name: statement}
            statements_to_process = list(discussion_statements.items())
        elif isinstance(discussion_statements, list):
            # List format: could be [{"player_name": "...", "statement": "..."}, ...] 
            # or [{player_name: statement, ...}, ...]
            for item in discussion_statements:
                if isinstance(item, dict):
                    # Check if it has the expected structure
                    if "player_name" in item and "statement" in item:
                        # Format: {"player_name": "...", "statement": "..."}
                        speaker = item.get("player_name", "")
                        text = item.get("statement", "")
                        if speaker and text:
                            statements_to_process.append((speaker, text))
                    else:
                        # Format: {player_name: statement, ...}
            ...
            print(f"Unexpected discussion_statements format: {type(discussion_statements)}")
            return []
        
        print(f"Analyzing {len(statements_to_process)} statements with LLM...")
        
        for i, (speaker, text) in enumerate(statements_to_process):
            ...
            # Debug output
            print(f"Processing statement from {speaker}: {text[:100]}...")
            ...
            try:
                ...
                print(f"LLM found {len(llm_behaviors)} behaviors for {speaker}")
            except Exception as e:
                print(f"Error analyzing statement from {speaker}: {e}")
                llm_behaviors = []
            
            # Add metadata to each behavior found
            for behavior in llm_behaviors:
                behavior.update({
                    "source": f"{phase_type} {phase_number}, {speaker}",
                    "file": game_filename,
                    "model": model,
                })
            ...
            # Progress indicator
            if (i + 1) % 10 == 0:
                print(f"Processed {i + 1}/{len(statements_to_process)} statements...")
    ...
    def process_game_file(self, file_path: Path) -> List[Dict]:
        ...
        for player_name in player_names:
            ...
            raw_model = self.player_model_mapping.get(player_name, "unknown_model")
            model = self.extract_model_from_name(raw_model)
            player_models[player_name] = model
            self.all_players.add(player_name)
            self.all_models.add(self.extract_model_from_name(model))
        
        # Process all phases (both night and day phases)
        all_behaviors = []
        game_filename = file_path.stem  # Get filename without extension
        
        # Process night phases
        night_phases = data.get("night_phases", [])
        for night_phase in night_phases:
            discussion_statements = night_phase.get("discussion_statements", {})
            if discussion_statements:
                night_number = night_phase.get("night_number", 1)
                behaviors = self.analyze_discussion_statements_enhanced(
                    discussion_statements, player_models, game_filename, "Night", night_number
                )
                all_behaviors.extend(behaviors)
        
        # Process day phases
        day_phases = data.get("day_phases", [])
        for day_phase in day_phases:
            discussion_statements = day_phase.get("discussion_statements", {})
            if discussion_statements:
                day_number = day_phase.get("day_number", 1)
                behaviors = self.analyze_discussion_statements_enhanced(
                    discussion_statements, player_models, game_filename, "Day", day_number
                )
                all_behaviors.extend(behaviors)
