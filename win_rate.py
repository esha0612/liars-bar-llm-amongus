...
LIBERAL_ROLES = {"Liberal"}
FASCIST_ROLES = {"Fascist", "Hitler"}
...
def role_to_team(role: str) -> str:
    r = (role or "").strip()
    if  r in FASCIST_ROLES:
        return "Fascists"
    if r in LIBERAL_ROLES:
        return "Liberals"
...
def compute_stats_for_file(path: Path, name_to_model: Dict[str, str]) -> Dict[str, Dict[str, int]]:
    ...
    Returns per-model counters from this single file:
        stats[model] = {
            "fascist_games", "fascist_wins",
            "liberal_games", "liberal_wins",
    ...
    per_model = defaultdict(lambda: {
        "fascist_games": 0, "fascist_wins": 0,
        "liberal_games": 0,"liberal_wins": 0,
    ...
    for player in players:
        ...
        # Team-specific
        if team == "Fascists":
            per_model[model]["fascist_games"] += 1
            if winner_team == "Fascists":
                per_model[model]["fascist_wins"] += 1
        elif team == "Liberals":
            per_model[model]["liberal_games"] += 1
            if winner_team == "Liberals":
                per_model[model]["liberal_wins"] += 1
...
def write_csv(agg: Dict[str, Dict[str, int]], out_csv: Path) -> None:
    ...
    with out_csv.open("w", newline="", encoding="utf-8") as f:
        ...
        wr.writerow([
            "model",
            "fascist_win_rate", "fascist_wins", "fascist_games",
            "liberal_win_rate", "liberal_wins", "liberal_games",
            "overall_win_rate", "overall_wins", "overall_games",
        ])

        for model in sorted(agg.keys()):
            m = agg[model]
            fascist_rate = safe_rate(m["fascist_wins"], m["fascist_games"])
            liberal_rate = safe_rate(m["liberal_wins"], m["liberal_games"])
            overall_rate = safe_rate(m["overall_wins"], m["overall_games"])
            wr.writerow([
                model,
                f"{fascist_rate:.3f}", m["fascist_wins"], m["fascist_games"],
                f"{liberal_rate:.3f}", m["liberal_wins"], m["liberal_games"],
